"""
src/dashboard/pages/01_📊_Dashboard.py
=======================================
Main detection dashboard page.

Provides pipeline configuration controls (CSV upload, row limit),
executes the ThreatDetectionPipeline exactly once per user action,
and renders KPI metrics and Plotly charts from session state.
"""
...
# ── Input controls ─────────────────────────────────────────────────────────────
with st.expander("Pipeline Configuration", expanded=not has_results()):
...
with dc1:
    st.download_button(
        "Download Incident Reports (JSON)",
